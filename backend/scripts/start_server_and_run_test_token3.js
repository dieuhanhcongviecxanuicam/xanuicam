...
(async ()=>{
  try{
    ...
    const userRes = await pool.query("SELECT id, username, full_name, avatar, role_id FROM users WHERE id = $1", [3]);
    if (!userRes.rows.length) throw new Error('export_tester not found by id 3');
    ...
    const res = await axios.post(`${API_BASE}/tasks/export`, { format: 'xlsx', filters: {}, password: 'x' }, { headers: { Authorization: `Bearer ${token}` }, responseType: 'arraybuffer' });
