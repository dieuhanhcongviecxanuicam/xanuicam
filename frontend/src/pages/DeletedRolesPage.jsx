...
const DeletedRolesPage = () => {
  ...
  useEffect(() => {
    const fetch = async () => {
      setLoading(true);
      try {
        const data = await apiService.getDeletedRoles({ limit: 1000 });
        setItems(data);
      } catch (e) {
        console.error(e);
      } finally {
  ...
  const handleRestore = async (id) => {
    try {
      await apiService.restoreDeletedRole(id);
      apiService.logEvent({ action: 'role.restored', resource_type: 'role', resource_id: id }).catch(()=>{});
      setItems(current => current.filter(i => i.id !== id));
      setNotification({ message: 'Khôi phục vai trò thành công.', type: 'success' });
    } catch (e) {
  ...
  return (
    <div>
      ...
      <h1 className="text-2xl font-bold mb-4">Vai trò đã xóa</h1>
          ...
          <tbody>
            {items.map(i => (
              <tr key={i.id} className="border-t">
                <td className="px-6 py-4">{i.role_name}</td>
                <td className="px-6 py-4">{i.description}</td>
                <td className="px-6 py-4">{i.deleted_by || i.deleted_by_name || ''}</td>
      ...
      <DeleteConfirmationModal
        ...
        message="Bạn có chắc muốn khôi phục vai trò này?"
      />
      {/* 'Quay lại trang Phân quyền' button */}
      <div className="mt-4">
        <a href="/roles" className="btn-secondary">Quay lại trang Phân quyền</a>
      </div>
    </div>
  );
};

export default DeletedRolesPage;
