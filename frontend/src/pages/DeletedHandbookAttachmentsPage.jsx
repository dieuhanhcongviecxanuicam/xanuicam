...
const DeletedHandbookAttachmentsPage = () => {
    ...
    useEffect(() => {
        const fetch = async () => {
            setLoading(true);
            try {
                const data = await apiService.getDeletedArticleAttachments({ limit: 500 });
    ...
    const handleRestore = async (id) => {
        if (!window.confirm('Khôi phục tệp này?')) return;
        try {
            await apiService.restoreDeletedArticleAttachment(id);
    ...
    return (
        <div className="p-6">
            <div className="flex items-center justify-between mb-4">
                <h1 className="text-2xl font-bold">Tệp đã xóa - Kho dữ liệu số</h1>
                <div>
...
export default DeletedHandbookAttachmentsPage;
